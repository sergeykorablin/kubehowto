...
Если вы хотите быстро познакомиться с основными возможностями и концепциями kubernetes - обратите внимание на [minikube](https://kubernetes.io/ru/docs/tasks/tools/install-minikube/) и не читайте дальше эту статью! :devil:
...
Что по моему мнению необходимо знать: базовое понимание работы с Linux, системами виртуализации, представление о контейнерах (например Docker), понимание принципов работы сетей и сетевых протоколов.

## Что за кубер и зачем это всё

Kubernetes - это оркестратор контейнеров разработанный компанией Google для создания  распределенных приложений. На самом деле kubernetes выполняет намного больше функций чем просто оркесчтровка, например: автообнаружение, балансировка нагрузки между узлами кластера и копиями приложений, мониторинг и поддержание состояния приложенией, хранение и распространение секретных данных, организация доступа к хранилищам и т.д.
...
$ sudo swapoff -a
```
и комментируем соттветствующую запись в /etc/fstab

В качестве гостевой операционной системы буду использовать Ubuntu 20.04. Вы можете использовать друшие ОС, поддерживаются rhel-based и debian-based ОС. 

> Изначально я пытался собрать кластер на базе Ubuntu 22.04, но мне не удалось добиться стабильной работы, служебные сервисы (поды) рандомно падали и перезапускались, работать с таким кластером оказалось невозможно.
...
* ClusterIssuer - глобальный Issuer, может создавать tls секреты в любом namespace-е.
* Issuer - namespace-зависим, настраивается для каждого namespase-а в котором будет создавать и обновлять tls секреты с сертификатами.

Методы проверки домена могут быть DNS и HTTP01. Чтобы использовать dns ваш dns-провайдер должен быть в списке поддерживаемых, см. список в официальной документации.
В примере будем использовать HTTP01, для этого 80й порт loadbalancer-а кластера должен быть доступен из интернет по доменому имени dev.example.com.
В моем случае кластер находится в локальной сети и не имеет публичного ip-адреса, поэтому на отдельном сервере который у меня торчит в интернет настраиваю nginx в качестве реверс-прокси для http://dev.example.com.
